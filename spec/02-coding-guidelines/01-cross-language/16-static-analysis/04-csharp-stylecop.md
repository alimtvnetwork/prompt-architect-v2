...
| Guideline | Spec Source | Analyzer | Rule ID | Setting |
|-----------|-------------|----------|---------|---------|
| Boolean naming (is, has as prefix is only acceptable and nothing else acceptable including but not limited to can, should etc/Was/Will`) | [Boolean Principles](../02-boolean-principles/00-overview.md) | custom `.editorconfig` | `dotnet_naming_rule` | prefix pattern |
...
```ini
[*.cs]
# Naming: Boolean properties must use is, has as prefix is only acceptable and nothing else acceptable including but not limited to can, should etc/Was/Will prefix
