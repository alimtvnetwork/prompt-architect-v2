...
| **06** | [`06-code-hygiene.md`](./06-code-hygiene.md) | Code Hygiene & Project Architecture | `cg-hygiene`, `cg-execute hygiene`, `audit hygiene` | File caps ($\le$ 100 lines coding max, rec $\le$ 80), functions ($\le$ 8–15 lines), zero nested `if`s, struct caps ($\le$ 120 lines), dedicated definition files | `check-file-sizes.py`, `check-placeholder-comments.py` |
...
- **Standard File Size:** Max 100 coding lines per file (recommended $\le$ 80 lines).
